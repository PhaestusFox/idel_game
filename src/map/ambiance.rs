...
fn spawn_sun(mut commands: Commands) {
    commands.spawn((
        ...
        Transform::from_xyz(10000., 4000., 0.0).looking_at(Vec3::ZERO, Vec3::Y),
        VolumetricLight,
    ));
    ...
    commands.spawn((
        DirectionalLight {
            ...
            // sun unfiltered by the atmosphere, so it is the proper input for
            // sunlight to be filtered by the atmosphere.
            illuminance: light_consts::lux::RAW_SUNLIGHT * (1. / 1_000.),
            ..default()
        },
        Transform::from_xyz(10000., -4000., 100.0).looking_at(Vec3::ONE, Vec3::Y),
        VolumetricLight,
    ));
}

const DAY_LENGTH: f32 = 100000.; //In seconds
